...
def make_case(root: Path, *, task_mode="edit", validators=None, allowed=None):
    return EvalCase(
        ...
        expected_skills=("dejavu-test-writer",),
        task_mode=task_mode,
        kind="direct",
        fixture="compose-jvm",
...
def make_result(workspace: Path, *, paths=(), events=(), output=None, returncode=0):
    return SubjectResult(
        ...
        events=tuple(events),
        final_output=output
        or {"summary": "done", "skills_used": ["dejavu-test-writer"], "evidence": ["diff"]},
        usage={},
...
python3 \""'$SKILL_DIR/scripts/gradle_run.py" create' ''',
    ...
    def test_subject_output_accepts_plugin_prefixed_skill_names(self):
        result = make_result(
            ...
            output={
                "summary": "done",
                "skills_used": ["dejavu:dejavu-test-writer"],
