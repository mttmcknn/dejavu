# Adapted for DejaVu: selected skill revision, bounded calls, evaluator fingerprint. See evals/upstream/README.md.
...
from evals.harness.skill_source import skill_root
...
def _case_digest(case: EvalCase) -> str:
    ...
    for label, root in roots:
        for path in sorted(path for path in root.rglob("*") if path.is_file()):
            ...
            digest.update(b"\0")
    for path in sorted((repo_root / "evals").rglob("*")):
        relative = path.relative_to(repo_root / "evals")
        if path.is_file() and relative.parts[0] in {"harness", "schemas", "validators"} and "__pycache__" not in relative.parts:
            digest.update(relative.as_posix().encode())
            digest.update(path.read_bytes())
...
def _skill_source_paths(repo_root: Path) -> tuple[Path, ...]:
    skill_dirs = (skill_root(repo_root) / skill for skill in PUBLIC_SKILLS)
    return tuple(
        sorted(
            (
                path.resolve()
    ...
    *,
    ...
    for case, arm in conditions:
        for repetition in range(1, repetitions + 1):
            ...
            # Bound call counts: failures are evidence, never hidden automatic retries.
            subject = recorded_subject_attempt()
            subject_retries = 0
