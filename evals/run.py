#!/usr/bin/env python3
# Adapted for DejaVu: default suite. See evals/upstream/README.md.
...
from evals.harness.skill_source import select_skill_revision


def _add_experiment_arguments(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--skills-ref", help="Compare a committed skill revision with the same current corpus/harness")
    ...
    parser.add_argument("--suite", choices=sorted(SUITES), default="dejavu")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    run.add_argument("--max-calls", type=int, default=12, help="Hard matrix cap; no automatic subject/judge retries")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command in {"plan", "run"}:
        ...
        if (costs[0] is None) != (costs[1] is None):
            ...
            return 2
        try:
            select_skill_revision(repo_root, args.skills_ref)
        ...
        if not execute:
            return 0
        if args.max_calls <= 0 or plan["total_calls"] > args.max_calls:
            print(f"planned {plan['total_calls']} calls exceed --max-calls={args.max_calls}", file=sys.stderr)
            return 2
